...
from pyvatsim.liveapi import Facility, VatsimLiveAPI, Rating, PilotRating, Server
...
class TestServerDataclass:
    def test_can_set_rating_class_properties_from_api(
        ...
        assert (
            result.client_connections_allowed == raw_server["client_connections_allowed"]
        )
        assert (
            result.clients_connection_allowed == raw_server["clients_connection_allowed"]
        )
